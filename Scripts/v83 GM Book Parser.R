...
prepare.name <- function(name) {
  name <- na.omit(name) %>% dQuote()
  name <- sub("“",'"',name)
  name <- sub("”",'"',name)
  name <- paste("{", name)
}
...
useItemsName <- prepare.name(useItemsName)
useItemsDF <- data.frame(useItemsID, useItemsName, stringsAsFactors = FALSE)
useItemPair <- paste(useItemsDF$useItemsName, useItemsDF$useItemsID, sep = ", ") %>% noquote() %>% paste("},")
...
setupItemsName <- prepare.name(setupItemsName)
setupItemsDF <- data.frame(setupItemsID, setupItemsName, stringsAsFactors = FALSE)
setupItemPair <- paste(setupItemsDF$setupItemsName, setupItemsDF$setupItemsID, sep = ", ") %>% noquote() %>% paste("},")
...
etcItemsName <- prepare.name(etcItemsName)
etcItemsDF <- data.frame(etcItemsID, etcItemsName, stringsAsFactors = FALSE)
etcItemPair <- paste(etcItemsDF$etcItemsName, etcItemsDF$etcItemsID, sep = ", ") %>% noquote() %>% paste("},")
...
cashItemsName <- prepare.name(cashItemsName)
cashItemsDF <- data.frame(cashItemsID, cashItemsName, stringsAsFactors = FALSE)
cashItemPair <- paste(cashItemsDF$cashItemsName, cashItemsDF$cashItemsID, sep = ", ") %>% noquote() %>% paste("},")
...
faceOrLookName <- prepare.name(faceOrLookName)
faceOrLookDF <- data.frame(faceOrLookID, faceOrLookName, stringsAsFactors = FALSE)
faceOrLookPair <- paste(faceOrLookDF$faceOrLookName, faceOrLookDF$faceOrLookID, sep = ", ") %>% noquote() %>% paste("},")
...
hatName <- prepare.name(hatName)
hatDF <- data.frame(hatID, hatName, stringsAsFactors = FALSE)
hatPair <- paste(hatDF$hatName, hatDF$hatID, sep = ", ") %>% noquote() %>% paste("},")
...
faceAccName <- prepare.name(faceAccName)
faceAccDF <- data.frame(faceAccID, faceAccName, stringsAsFactors = FALSE)
faceAccPair <- paste(faceAccDF$faceAccName, faceAccDF$faceAccID, sep = ", ") %>% noquote() %>% paste("},")
...
eyeAccName <- prepare.name(eyeAccName)
eyeAccDF <- data.frame(eyeAccID, eyeAccName, stringsAsFactors = FALSE)
eyeAccPair <- paste(eyeAccDF$eyeAccName, eyeAccDF$eyeAccID, sep = ", ") %>% noquote() %>% paste("},")
...
earringName <- prepare.name(earringName)
earringDF <- data.frame(earringID, earringName, stringsAsFactors = FALSE)
earringPair <- paste(earringDF$earringName, earringDF$earringID, sep = ", ") %>% noquote() %>% paste("},")
...
topName <- prepare.name(topName)
topDF <- data.frame(topID, topName, stringsAsFactors = FALSE)
topPair <- paste(topDF$topName, topDF$topID, sep = ", ") %>% noquote() %>% paste("},")
...
bottomName <- prepare.name(bottomName)
bottomDF <- data.frame(bottomID, bottomName, stringsAsFactors = FALSE)
bottomPair <- paste(bottomDF$bottomName, bottomDF$bottomID, sep = ", ") %>% noquote() %>% paste("},")
...
shoesName <- prepare.name(shoesName)
shoesDF <- data.frame(shoesID, shoesName, stringsAsFactors = FALSE)
shoesPair <- paste(shoesDF$shoesName, shoesDF$shoesID, sep = ", ") %>% noquote() %>% paste("},")
...
glovesName <- prepare.name(glovesName)
glovesDF <- data.frame(glovesID, glovesName, stringsAsFactors = FALSE)
glovesPair <- paste(glovesDF$glovesName, glovesDF$glovesID, sep = ", ") %>% noquote() %>% paste("},")
...
capeName <- prepare.name(capeName)
capeDF <- data.frame(capeID, capeName, stringsAsFactors = FALSE)
capePair <- paste(capeDF$capeName, capeDF$capeID, sep = ", ") %>% noquote() %>% paste("},")
...
shieldName <- prepare.name(shieldName)
shieldDF <- data.frame(shieldID, shieldName, stringsAsFactors = FALSE)
shieldPair <- paste(shieldDF$shieldName, shieldDF$shieldID, sep = ", ") %>% noquote() %>% paste("},")
...
weaponName <- prepare.name(weaponName)
weaponDF <- data.frame(weaponID, weaponName, stringsAsFactors = FALSE)
weaponPair <- paste(weaponDF$weaponName, weaponDF$weaponID, sep = ", ") %>% noquote() %>% paste("},")
...
ringName <- prepare.name(ringName)
ringDF <- data.frame(ringID, ringName, stringsAsFactors = FALSE)
ringPair <- paste(ringDF$ringName, ringDF$ringID, sep = ", ") %>% noquote() %>% paste("},")
...
necklaceName <- prepare.name(necklaceName)
necklaceDF <- data.frame(necklaceID, necklaceName, stringsAsFactors = FALSE)
necklacePair <- paste(necklaceDF$necklaceName, necklaceDF$necklaceID, sep = ", ") %>% noquote() %>% paste("},")
...
beltName <- prepare.name(beltName)
beltDF <- data.frame(beltID, beltName, stringsAsFactors = FALSE)
beltPair <- paste(beltDF$beltName, beltDF$beltID, sep = ", ") %>% noquote() %>% paste("},")
...
petEquipName <- prepare.name(petEquipName)
petEquipDF <- data.frame(petEquipID, petEquipName, stringsAsFactors = FALSE)
petEquipPair <- paste(petEquipDF$petEquipName, petEquipDF$petEquipID, sep = ", ") %>% noquote() %>% paste("},")
...
mountName <- prepare.name(mountName)
mountsDF <- data.frame(mountID, mountName, stringsAsFactors = FALSE)
mountPair <- paste(mountsDF$mountName, mountsDF$mountID, sep = ", ") %>% noquote() %>% paste("},")
